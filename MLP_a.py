...
def MLP_Assembly(optimizer, loss_function, X_train, y_train, X_test, y_test, epoch, Nodes, activation, counter, num_lab, b, layer_number):
  ...
  print(accuracy_list)
  df = pd.DataFrame(list(zip(optimizer_list, loss_function_list, epoch_list, node_1_length_list, activation_layer_1_list, activation_layer_2_list, accuracy_list, layer_number_list)),
                        columns =['optimizer', 'loss_function', "epochs", "node1_length", "activation_layer1" , "activation_layer2", "perceptage_misclassified", "layer_number"])
  return df

#define variables
#Nodes = np.arange(50, 2050, 500)
Nodes = [750]
#Nodes = np.arange(10, 3060, 50)
