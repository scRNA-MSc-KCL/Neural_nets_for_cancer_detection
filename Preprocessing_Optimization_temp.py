...
def SVM_Optimizer_Method_2(data, labels, filter_genes, normalize, filter_by_highly_variable_gene, unit_var, log):
  filter_genes_list = []
  normalize_list = []
      ...
      #filter genes
        for d in filter_by_highly_variable_gene:
          ...
          for e in unit_var:
            ...
            percentage_accuracy_list.append(percentage_accuracy) 
            data_shape_list.append(filtered_3_data.shape)
    df = pd.DataFrame(list(zip(filter_genes_list, normalize_list, filter_method, filter_by_highly_variable_genes_list, unit_var_list, log_list, percentage_accuracy_list, data_shape_list)),
                          columns =['Min_number_of_cells_per_gene', 'normalized', "filter_method", "number_of_top_genes", "scaled_to_unit_var", "logarithmized", "percentage_accuracy", "data_shape"])
  return df      
...
log = ["yes", "no"]
#normalize = ["no"]
filter_by_highly_variable_gene = [1000, 2000]
