...
#Build CNN
    net = Sequential()
    net.add(Conv2D(filters=64, kernel_size=(7,7), activation='relu',input_shape=(p,p,3)))
...
df = pd.DataFrame(list(zip(accuracy_list, run_time_list, pixel_list, feature_list,)),columns =['accuracy', 'run_time', 
                               'pixels', 'features'])
