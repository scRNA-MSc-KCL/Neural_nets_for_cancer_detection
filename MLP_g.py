...
def MLP_Assembly(optimizer, loss_function, X_train, y_train, X_test, y_test, X_val, y_val, epoch, Nodes, activation, counter, num_lab, b, layer_number, kernal_init):
  optimizer_list = []
  loss_function_list = []
  epoch_list = []
  node_1_length_list = []
          ...
          for a1 in activation:
            for lr in layer_number:
              for k in kernal_init:
                net = Sequential()
                if lr == 1:
                  net.add(Dense(n1, activation = a1, kernel_initializer = k, input_shape = (data.n_vars,)))
                if lr == 2:
                  net.add(Dense(n1, activation = a1,kernel_initializer = k, input_shape = (data.n_vars,)))
                  net.add(Dense(n1, activation = a1, kernel_initializer = k))
                if lr == 3:
                  net.add(Dense(n1, activation = a1, kernel_initializer = k, input_shape = (data.n_vars,)))
                  net.add(Dense(n1, activation = a1, kernel_initializer = k))
                  net.add(Dense(n1, activation = a1, kernel_initializer = k))
  ...
  print(accuracy_list)
  df = pd.DataFrame(list(zip(optimizer_list, loss_function_list, epoch_list, node_1_length_list, activation_layer_1_list, accuracy_list, layer_number_list, kernal_initializer_list)),
                          columns =['optimizer', 'loss_function', "epochs", "node1_length", "activation_layer1", "perceptage accuracy", "layer_number", "kernal"])
  return df
